...
const addGrade = async (req, res) => {
  ...
  try {
    ...
    if (response.acknowledged > 0) {
      ...
      res
        .status(500)
        .json(response.error || "Some error occurred while creating the grade");
    }
  } catch (err) {
    res.status(400).json({ message: err });
  }
...
const updateGrade = async (req, res) => {
  ...
  try {
    ...
    const response = await mongodb
      ...
      .db()
      .collection("grades")
      .replaceOne({ _id: gradeId }, grade);
    if (response.modifiedCount > 0) {
      res.status(204).send();
    } else {
      res
        .status(500)
        .json(response.error || "Some error occurred while updating the grade");
    }
  } catch (err) {
    res.status(400).json({ message: err });
  }
...
const deleteGrade = async (req, res) => {
  ...
  try {
    ...
    if (response.deletedCount > 0) {
      ...
      res
        .status(500)
        .json(response.error || "Some error occurred while deleting the grade");
    }
  } catch (err) {
    res.status(400).json({ message: err });
  }
